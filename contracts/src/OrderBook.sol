...
/// @title OrderBook
/// @notice On-chain limit order book for binary outcome tokens. This is where
///         Alpha Market forms its own odds, rather than mirroring a price.
///
...
contract OrderBook is Ownable, ReentrancyGuard, Pausable {
    ...
    function placeOrder(
        ...
        if (!registry.isTradeable(marketId)) revert MarketNotTradeable(marketId);
    ...
    function _live(uint256 orderId) internal view returns (Order storage o) {
        ...
        if (!registry.isTradeable(o.marketId)) revert MarketNotTradeable(o.marketId);
    }
